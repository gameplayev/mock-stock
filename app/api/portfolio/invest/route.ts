...
export async function POST(request: NextRequest) {
  try {
    ...
    if (Number.isNaN(numericShares) || Number.isNaN(numericPrice) || numericShares <= 0 || numericPrice <= 0) {
    ...
    const orderValue = numericShares * numericPrice;
    const now = new Date();
    const expiresAt = new Date(now.getTime() + POSITION_DURATION_MS).toISOString();
    const targetIndex = user.holdings.findIndex(
      (holding: HoldingDocument) => holding.symbol === symbol && holding.direction === "long",
    );
    ...
    if (action === "buy") {
      ...
      if (target) {
        const totalShares = target.shares + numericShares;
      ...
      if (!target || target.shares < numericShares) {
        return NextResponse.json({ message: "보유 수량이 부족합니다." }, { status: 400 });
      }

      target.shares -= numericShares;
      target.price = numericPrice;
      target.change = 0;
